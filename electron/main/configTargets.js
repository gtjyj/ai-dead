...
function updateCodexConfig(rawContent, api) {
  ...
  const preservedTopLevelLines = topLevelLines.filter(line => {
    ...
    return trimmedLine && !trimmedLine.startsWith('model_provider = ') && !trimmedLine.startsWith('model = ');
  });
  ...
  const withoutProviderTable = stripTomlTable(tableLines, `model_providers.${CODEX_PROVIDER_KEY}`);
  const rebuiltLines = [
...
function buildOpenCodeProvider(api, providerKey) {
  ...
  if (vendor === 'openai' || vendor === 'gemini' || vendor === 'anthropic' || vendor === 'other') {
...
async function applyApiToCodex(apiId) {
  ...
  const authConfig = parseJsonOrDefault(rawAuth, {}, 'Codex auth.json 不是有效的 JSON。');
  ...
  await Promise.all([
    ...
    fs.writeFile(authFilePath, `${JSON.stringify(nextAuthConfig, null, 2)}\n`, 'utf8'),
  ]);
...
async function applyApiToOpenCode(apiId) {
  ...
  const config = parseJsonOrDefault(rawConfig, {}, 'OpenCode 配置文件不是有效的 JSON。');
  ...
  await fs.writeFile(configFilePath, `${JSON.stringify(nextConfig, null, 2)}\n`, 'utf8');
